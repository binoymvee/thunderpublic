...
def installOpenstackOS(nodeInfo, envName):
    ...
    if not nodeInfo.currentos:
        ...
        try:
            ...
            nicName, deviceIp = getNodeNetworkNIC(nodeInfo.id, "A")
            ...
            cobblerCmd += " --ip-address=" + nodeInfo.node_ip + " --hostname=" + hostName + " --interface=" + nicName
...
def getNodeChefDeploymentCommand(nodeInfo, envName = '', chefInstall = False):
    ...
    if chefInstall:
        ...
        chefCommand += " --bootstrap-install-command 'type chef-client >/dev/null 2>&1 || { wget " + downloadLink + "; dpkg -i " + chefPkgName + "; rm -f /etc/chef/client.pem;}'"
