...
def getNodeNetworkNIC(nodeId, networkType):
    ...
    sql = """
    select nic.name, nmap.ip_address from thunder_network_interface nic, thunder_nic_mapping nmap
...
def verifyIPConnection(ipAddress, fromIpAddr, networkInfo, cloudId, networkType, netmask, vlanTag = False):
    ...
    if nodeList:
        ...
        for nodeInfo in nodeList:
            ...
            nicName, deviceIp = getNodeNetworkNIC(nodeInfo.id, nicType)
            ...
            if index == 1:
                ...
                overrideList['thunder']['ip'] = settings.THUNDER_HOST
